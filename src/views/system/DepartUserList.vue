      ...
      <a-card :bordered="false">
        <div style="background: #fff;padding-left:16px;height: 100%; margin-top: 5px">
          ...
          <div style="margin-top: 24px;" v-else-if="userIdentity === '2' && departTree.length==0">
            <h3><span>あなたの部門に有効な部門情報がありません</span></h3>
          </div>
          <div style="margin-top: 24px;" v-else><h3>一般従業員の一時的な許可</h3></div>
        </div>
      </a-card>
    </a-col>
    <a-col :md="16" :sm="24">
    ...
    methods: {
      ...
      setThisExpandedKeys(node) {
        ...
        if (node.children && node.children.length > 0) {
          ...
          //以下のコードは、コメントをリリースすると、デフォルトですべてのノードを展開します
          /**
