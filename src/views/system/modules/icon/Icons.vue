<template>
  <a-modal
    ...
    <a-tabs>
      ...
      <a-tab-pane tab="ウェブサイトのユニバーサルアイコン" key="5">
        <ul>
...
export default {
  ...
  methods: {
    ...
    chooseIcon (icon) {
      ...
      this.$message.success(`選択済み ${icon}`)
    },
    ok () {
      if (this.choosedIcon === '') {
        this.$message.warning('アイコンが選択されていません')
        return
      }
      this.reset()
