  ...
  <div>
    <a-form :form="form" style="max-width: 500px; margin: 40px auto 0;" @keyup.enter.native="nextStep">
      ...
      <a-form-item :wrapperCol="{span: 19, offset: 5}">
        <router-link style="float: left;line-height: 40px;" :to="{ name: 'login' }">既存のアカウントでログインします</router-link>
        <a-button type="primary" @click="nextStep" style="margin-left: 20px">次へ</a-button>
      </a-form-item>
    </a-form>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        validatorRules: {
          captcha: {rule: [{required: true, message: 'SMS確認コードを入力してください!'}, {validator: this.validateCaptcha}]},
          phone: {rule: [{required: true, message: '電話番号を入力してください!'}, {validator: this.validatePhone}]},
        },
      }
    },
    computed: {
    ...
    methods: {
          ...
          if (!err) {
            if (that.dropList == "0") {
              if (values.captcha == undefined) {
                this.cmsFailed("SMS確認コードを入力してください!");
              } else {
                var params = {}
      ...
      getCaptcha(e) {
        ...
        if(!phone){
          this.cmsFailed("電話番号を空にすることはできません!");
          return;
        }
