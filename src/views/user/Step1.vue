    ...
    <a-form style="max-width: 500px; margin: 40px auto 0;" :form="form" @keyup.enter.native="nextStep">
      <a-form-item>
        <a-input
          ...
          type="text"
          autocomplete="false"
          placeholder="ユーザーアカウントまたは携帯電話番号を入力してください">
      ...
      <a-form-item :wrapperCol="{span: 19, offset: 5}">
        ...
        <a-button type="primary" @click="nextStep">次へ</a-button>
      </a-form-item>
    </a-form>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      validateInputUsername(rule, value, callback) {
        ...
        var reg = /^[0-9]+.?[0-9]*/;
        if (!value) {
          callback("ユーザー名と携帯電話番号を入力してください！");
        }

        //ユーザーがアカウントまたは携帯電話番号を入力するかどうかを決定します
        if (reg.test(value)) {
          var params = {
            phone: value,
          };
